...
export class AchatDetailsComponent implements OnInit {
  ...
  total = computed(() => {
    return this.achatsDetails().reduce((sum, item) => sum + item.prix * item.quantite, 0);
  });

  dateRecuperation = computed(() => {
    const details = this.achatsDetails();
    return details.length > 0 ? details[0].panier?.dateHeureRecuperation : null;
  });

  annuler = computed(() => {
    const dateStr = this.dateRecuperation();
    if (!dateStr) return false;
    const dateRecup = new Date(dateStr);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return dateRecup >= today;
  });
